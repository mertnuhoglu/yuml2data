...
main_ddl_to_data = function() {
  ...
  ddl_to_data.sh = system.file("bash/ddl_to_data.sh", package = "yuml2data")
  data_model_dir = env_data_model_dir()
...
rdb_to_id_entity = function(data_model_dir, entity_names) {
  for (entity_name in entity_names) {
    ...
    writeLines(as.character(id_data), sprintf("%s/rdb/view/id_%s.txt", data_model_dir, entity_name))
  }
}
